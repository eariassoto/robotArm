...
// Modificado para mostrar un ejemplo de un ciclo en la interfaz gráfica de
// glut.
...
// ------------------------------------------------
// Funciones usadas para implementar el ejemplo del ciclo
...
void regularKey(unsigned char key, int x, int y) {
  if (miArchivo != NULL) {
    switch (key) {
      case GLUT_KEY_ENTER: iniciaCiclo(miArchivo); break;
      default: return;
    }
  }
  else{
      printf("Nombre de archivo de instrucciones inválido\n");
  }
}
...
int main(int argc, char** argv) {
  ...
  glutCreateWindow("Left Arm of the Forbidden One");
